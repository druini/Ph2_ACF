...
int main ( int argc, char* argv[] )
{
  ...
  cmd.defineOption ( "control-file", "Hw Description file for Control (electrical) FC7. Default value: settings/D19CDescription_ROH_EFC7.xml", ArgvParser::OptionRequiresValue /*| ArgvParser::OptionRequired*/ );
  cmd.defineOptionAlternative ( "control-file", "cf" );
  cmd.defineOption ( "backend-file", "Hw Description file for Back-End (optical) FC7. Default value: settings/D19CDescription_ROH_OFC7.xml", ArgvParser::OptionRequiresValue | ArgvParser::OptionRequired );
  ...
  if(cmd.foundOption("internal-pattern") || cmd.foundOption("external-pattern"))
  {
    ...
    {
      cBackEndROHHybridTester.TestULInternalPattern(cInternalPattern32);
    }
