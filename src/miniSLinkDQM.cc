...
#if 1
	    std::cout << setw(4) << cData.size()
                      << "\t"
                      << std::bitset<64> (correctedWord) 
		      << "\t0x"
		      << std::hex
		      << std::noshowbase
		      << std::setw(16)
		      ...
		      << correctedWord
	              << std::setfill(' ')
                      << std::dec
                      << "\t" << ((correctedWord & 0xFF00000000000000) >> 56 == 0xa0)
	              << "\t" << ((correctedWord & 0x00000000000000F0) >> 4 == 0x7)
		      << std::endl;
#endif
...
int main ( int argc, char* argv[] )
{
    ...
    std::unique_ptr<SLinkDQMHistogrammer> dqmH 
        = std::unique_ptr<SLinkDQMHistogrammer> (new SLinkDQMHistogrammer (0, addTree, skipHist) );
