...
struct Named
{
    using names_tuple = std::tuple<Names...>;
    static constexpr const char* _names[] = {Names::value...};
    static constexpr uint32_t _hashes[] = {Names::hash...};
    ...
    struct Tuple 
    {
        ...
        static constexpr auto& hashes = _hashes;

        template <class T = Tuple, typename std::enable_if_t<(std::tuple_size<typename T::value_type>::value > 0), int> = 0>
        Tuple() {}

        template <class... Us, std::enable_if_t<std::is_constructible<value_type, Us...>::value, int> = 0>
        constexpr Tuple(Us&&... values) : _values(std::forward<Us>(values)...) {}

        template <class Name>
        static constexpr size_t index = 
            detail::find(std::begin(_hashes), std::end(_hashes), Name::hash) - std::begin(_hashes);
        ...
        constexpr std::tuple<Ts...>& values() { return _values; }
        constexpr const std::tuple<Ts...>& values() const { return _values; }
...
template <class... Names>
const char* const Named<Names...>::_names[];

template <class... Names, class... Ts>
constexpr auto named_tuple(const std::pair<Names, Ts>&... fields) {
    return typename Named<Names...>::template Tuple<Ts...>(fields.second...);
}
