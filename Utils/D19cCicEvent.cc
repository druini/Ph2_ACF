...
namespace Ph2_HwInterface {
    ...
    D19cCicEvent::D19cCicEvent ( const BeBoard* pBoard,  uint32_t pNbCic, uint32_t pNFe, const std::vector<uint32_t>& list )
    : fEventDataVector(pNbCic*pNFe) , fEventDataList(pNbCic*pNFe) 
    {
    ...
    void D19cCicEvent::SetEvent ( const BeBoard* pBoard, uint32_t pNbCbc, const std::vector<uint32_t>& list )
    {
        ...
        do
        {
            ...
            {
                LOG (INFO) << BOLDRED << "Weird Stub header for hybrid" << +cFeId << " - stub : " << std::bitset<32>(*(cIterator) ) << " - header is " << std::bitset<4>(cStubHeader) << " -- expect  " << +cStubDataSize << " 128 bits words." <<  RESET;
            }
            ...
            for( auto cWord : cL1data )
            {
                ...
                if( sL1packet.size() > 274 ) 
                {
                    if( cReadoutChipId < 8 ) 
                    {
                        ...
                        //LOG (DEBUG) << BOLDBLUE << "\t... remaining word to go into next packet is : " << cRemainder << RESET; 
                        cReadoutChipId+=1;
                    }
                }
            }
    ...
    // NOT READY (what is i??????????)
    bool D19cCicEvent::Error ( uint8_t pFeId, uint8_t pCbcId, uint32_t i ) const
    {
    ...
    uint32_t D19cCicEvent::BxId ( uint8_t pFeId ) const
    {
        return fEventDataList[pFeId].first.first.first;
    }
    uint16_t D19cCicEvent::Status ( uint8_t pFeId ) const
    {
        return fEventDataList[pFeId].first.first.second;
    }
    ...
    {
        auto cIndex = 7 - std::distance( fFeMapping.begin() , std::find( fFeMapping.begin(), fFeMapping.end() , pReadoutChipId ) ) ; 
        LOG (DEBUG) << BOLDBLUE << "Decoding pipeline address from chip " << +pReadoutChipId << " on FeHybrid" << +pFeId << " this is stored in " <<  cIndex << " , event list has " << fEventDataList[pFeId].second.size() << " entries." << RESET;
        auto& cDataBitset = fEventDataList[pFeId].second[ cIndex ];
        std::bitset<274> cMask;  
        // L1 data is : 2 error bits + 9 pipeline address + 9 L1A + 254 hit
        // data 
    ...
    bool D19cCicEvent::DataBit ( uint8_t pFeId, uint8_t pReadoutChipId, uint32_t i ) const
    {
        ...
        if ( i >= NCHANNELS )
            return 0;
        std::bitset<274> cMask; cMask.set(NCHANNELS-1-i);
    ...
    std::string D19cCicEvent::GlibFlagString ( uint8_t pFeId, uint8_t pCbcId ) const
    {
        return "";
    }
