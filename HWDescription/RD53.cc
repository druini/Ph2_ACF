...
RD53::RD53(const FrontEndDescription& pFeDesc, uint8_t pRD53Id, const std::string& fileName)
    : ReadoutChip(pFeDesc, pRD53Id)
{
...
RD53::RD53(uint8_t pBeId, uint8_t pFMCId, uint8_t pFeId, uint8_t pRD53Id, const std::string& fileName)
            ...
            if (line.find_first_not_of(" \t") == std::string::npos || line.at(0) == '#' || line.at(0) == '*' || line.empty())
                ...
                if (line.find("COL") != std::string::npos) {
                    ...
                    if (row < nRows) {
                        myString.str("");
                        myString.clear();
                        myString << "[RD53::loadfRegMap]\tError, problem reading RD53 config file: too few rows (" << row
                                 << ") for column " << fPixelsMask.size();
...
void RD53::saveRegMap(const std::string& fName2Append)
{
    ...
    if (file) {
        ...
        for (const auto& v : fSetRegItem) {
            ...
            file << "0x" << std::setfill('0') << std::setw(2) << std::hex << std::uppercase << int(v.second.fAddress) << "\t0x"
                 << std::setfill('0') << std::setw(4) << std::hex << std::uppercase << int(v.second.fDefValue) << "\t\t\t0x"
                 << std::setfill('0') << std::setw(4) << std::hex << std::uppercase << int(v.second.fValue) << "\t\t\t"
...
void RD53::copyMaskFromDefault()
{
    for (auto i = 0u; i < fPixelsMask.size(); i++) {
        ...
        for (auto j = 0u; j < fPixelsMask[i].TDAC.size(); j++)
            fPixelsMask[i].TDAC[j] = fPixelsMaskDefault[i].TDAC[j];
    }
}
...
void RD53::copyMaskToDefault()
{
    for (auto i = 0u; i < fPixelsMaskDefault.size(); i++) {
        ...
        for (auto j = 0u; j < fPixelsMaskDefault[i].TDAC.size(); j++)
            fPixelsMaskDefault[i].TDAC[j] = fPixelsMask[i].TDAC[j];
    }
}
...
void RD53::resetMask()
{
    for (auto i = 0u; i < fPixelsMask.size(); i++) {
        ...
        for (auto j = 0u; j < fPixelsMask[i].TDAC.size(); j++)
            fPixelsMask[i].TDAC[j] = this->setBits(RD53EvtEncoder::NBIT_TOT / NPIX_REGION) / 2;
    }
}
...
void RD53::encodeCMD(const uint16_t address, const uint16_t data, const uint8_t RD53id, const uint16_t RD53cmd,
                     const bool isBroadcast, std::vector<uint32_t>& pVecReg, const std::vector<uint16_t>* dataVec)
{
    uint8_t FWcmd;
    uint32_t word = 0;
    std::bitset<NBIT_FRAME> frame(0);

    if ((RD53cmd == RD53CmdEncoder::RESET_ECR) || (RD53cmd == RD53CmdEncoder::RESET_BCR) || (RD53cmd == RD53CmdEncoder::NOOP)) {
        ...
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(1) << NBIT_DATA * NDATAMAX_PERPIXEL - 1))
              >> NBIT_FRAME * 19;
        frame = tmp.to_ulong() | ((address & this->setBits(NBIT_ID)) << 1); // @TMP@ A[3..0],D[95]
        word = word | (frame.to_ulong() << (NBIT_5BITW + NBIT_CMD / 2 + NBIT_FRAME * 2));
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 18))
              >> NBIT_FRAME * 18; // D[94..90]
        word = word | (tmp.to_ulong() << (NBIT_5BITW + NBIT_CMD / 2 + NBIT_FRAME * 3));
        pVecReg.push_back(word);

        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 17))
              >> NBIT_FRAME * 17; // D[89..85]
        word = tmp.to_ulong() << NBIT_FRAME * 0;
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 16))
              >> NBIT_FRAME * 16; // D[84..80]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 1);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 15))
              >> NBIT_FRAME * 15; // D[79..75]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 2);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 14))
              >> NBIT_FRAME * 14; // D[74..70]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 3);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 13))
              >> NBIT_FRAME * 13; // D[69..65]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 4);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 12))
              >> NBIT_FRAME * 12; // D[64..60]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 5);
        pVecReg.push_back(word);

        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 11))
              >> NBIT_FRAME * 11; // D[59..55]
        word = tmp.to_ulong() << NBIT_FRAME * 0;
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 10))
              >> NBIT_FRAME * 10; // D[54..50]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 1);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 9))
              >> NBIT_FRAME * 9; // D[49..45]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 2);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 8))
              >> NBIT_FRAME * 8; // D[44..40]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 3);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 7))
              >> NBIT_FRAME * 7; // D[39..35]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 4);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 6))
              >> NBIT_FRAME * 6; // D[34..30]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 5);
        pVecReg.push_back(word);

        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 5))
              >> NBIT_FRAME * 5; // D[29..25]
        word = tmp.to_ulong() << NBIT_FRAME * 0;
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 4))
              >> NBIT_FRAME * 4; // D[24..20]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 1);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 3))
              >> NBIT_FRAME * 3; // D[19..15]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 2);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 2))
              >> NBIT_FRAME * 2; // D[14..10]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 3);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 1))
              >> NBIT_FRAME * 1; // D[9..5]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 4);
        tmp = (dataBitStream & (this->setBits<NBIT_DATA * NDATAMAX_PERPIXEL>(NBIT_FRAME) << NBIT_FRAME * 0))
              >> NBIT_FRAME * 0; // D[4..0]
        word = word | (tmp.to_ulong() << NBIT_FRAME * 5);
    }
...
RD53::Event::Event(const uint32_t* data, size_t n)
{
    ...
    std::tie(header, trigger_id, trigger_tag, bc_id) = unpack_bits<RD53EvtEncoder::NBIT_HEADER, RD53EvtEncoder::NBIT_TRIGID,
                                                                   RD53EvtEncoder::NBIT_TRGTAG, RD53EvtEncoder::NBIT_BCID>(*data);
    if (header != RD53EvtEncoder::HEADER)
        evtStatus |= RD53EvtEncoder::CHEAD;

    size_t noHitToT = RD53::setBits(RD53EvtEncoder::NBIT_TOT);
    for (auto i = 1u; i < n; i++)
        if (data[i] != noHitToT) {
            this->data.emplace_back(data[i]);
            if ((this->data.back().row >= RD53::nRows) || (this->data.back().col >= RD53::nCols))
                evtStatus |= RD53EvtEncoder::CPIX;
        }
}

int RD53::Event::hit_count() const
{
    return std::accumulate(data.begin(), data.end(), 0, [](int s, const HitData& hit) {
        // std::cout << "row = " << hit.row << "\n";
        return s + std::count_if(hit.tots.begin(), hit.tots.end(), [](uint8_t tot) {
                //    std::cout << "asd" << (int)tot << "\n";
                   return tot != 15;
               });
    });
}
...
RD53::HitData::HitData(const uint32_t data)
{
    ...
    std::tie(core_col, row, side, all_tots)
        = unpack_bits<RD53EvtEncoder::NBIT_CCOL, RD53EvtEncoder::NBIT_ROW, RD53EvtEncoder::NBIT_SIDE, RD53EvtEncoder::NBIT_TOT>(
            data);
...
RD53::CalCmd::CalCmd(const uint8_t& cal_edge_mode, const uint8_t& cal_edge_delay, const uint8_t& cal_edge_width,
                     const uint8_t& cal_aux_mode, const uint8_t& cal_aux_delay)
...
void RD53::CalCmd::setCalCmd(const uint8_t& _cal_edge_mode, const uint8_t& _cal_edge_delay, const uint8_t& _cal_edge_width,
...
{
    return pack_bits<NBIT_ID, RD53InjEncoder::NBIT_CAL_EDGE_MODE, RD53InjEncoder::NBIT_CAL_EDGE_DELAY,
                     RD53InjEncoder::NBIT_CAL_EDGE_WIDTH, RD53InjEncoder::NBIT_CAL_AUX_MODE, RD53InjEncoder::NBIT_CAL_AUX_DELAY>(
        chipId, cal_edge_mode, cal_edge_delay, cal_edge_width, cal_aux_mode, cal_aux_delay);
}
} // namespace Ph2_HwDescription
