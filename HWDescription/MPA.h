...
namespace Ph2_HwDescription
 {
	...
	{
		    if(dacName.find("TrimDAC_P",0,9)!=std::string::npos ) return true;
		    else return false;
        }
        uint8_t getNumberOfBits(const std::string &dacName) override 
	{
		    if(dacName.find("TrimDAC_P",0,9)!=std::string::npos) return 5;
		    else return 8;
        }


    	std::pair<uint32_t, uint32_t>  PNlocal ( const uint32_t PN) 
    	{
		return std::pair<uint32_t, uint32_t>(PN/120,PN%120);
    	}

	uint32_t getNumberOfChannels() const override { return NMPACHANNELS; }


    uint32_t  PNglobal ( std::pair<uint32_t, uint32_t> PC ) 
    {
	return PC.first*120+PC.second;
    }
