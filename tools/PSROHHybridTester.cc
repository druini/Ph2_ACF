...
void PSROHHybridTester::Initialise ( )
{
    ...
    {
        fBeBoardInterface->WriteBoardReg (pBoard, "fc7_daq_ctrl.physical_interface_block.fe_for_ps_roh.i2c_slave_reset", 0x01);
    }
}
...
void PSROHHybridTester::ClearBRAM(const std::string & sBramToReset )
{
        for (auto pBoard : *fDetectorContainer)
        {
                this->ClearBRAM(pBoard, sBramToReset);
        }
}
...
//        this -> UserFCMDTranslate(filename);
	...
	if ( cIsSSAlFCMDBRAMGood ) LOG(INFO) << "SSA l reference FCMD writing to BRAM ->" << BOLDGREEN << " Successful" << RESET;
	else{ 
        ...
        if ( cIsSSArFCMDBRAMGood ) LOG(INFO) << "SSA r reference FCMD writing to BRAM ->" << BOLDGREEN << " Successful" << RESET;
        else{
        ...
        if ( cIsCIClFCMDBRAMGood ) LOG(INFO) << "CIC l reference FCMD writing to BRAM ->" << BOLDGREEN << " Successful" << RESET;
        else{
        ...
        if ( cIsCICrFCMDBRAMGood ) LOG(INFO) << "CIC r reference FCMD writing to BRAM ->" << BOLDGREEN << " Successful" << RESET;
        else{
...
void PSROHHybridTester::WritePatternToBRAM(const std::string & sFileName = "fcmd_file.txt")
{
        for (auto pBoard : *fDetectorContainer)
        {
                this->WritePatternToBRAM(pBoard, sFileName);
        }
}
...
void PSROHHybridTester::CheckFastCommandsBRAM(const std::string & sFCMDLine)
{
        for (auto pBoard : *fDetectorContainer)
        {
                this->CheckFastCommandsBRAM(pBoard, sFCMDLine);
        }
}
...
void PSROHHybridTester::CheckFastCommands(BeBoard* pBoard, const std::string & sFastCommand, const std::string & filename = "fcmd_file.txt")
{
	    ...
	    auto NTrials = 0;
            ...
            else LOG(INFO) << "SSA l FCMD test ->" << BOLDGREEN << " time out" << RESET;
	    ...
	    NTrials = 0;
            ...
            else LOG(INFO) << "SSA r FCMD test ->" << BOLDGREEN << " time out" << RESET;
	    ...
	    NTrials = 0;
            ...
            else LOG(INFO) << "CIC l FCMD test ->" << BOLDGREEN << " time out" << RESET;
	    ...
	    NTrials = 0;
            ...
            else LOG(INFO) << "CIC r FCMD test ->" << BOLDGREEN << " time out" << RESET;
...
void PSROHHybridTester::CheckFastCommands(const std::string & sFastCommand, const std::string & filename = "fcmd_file.txt")
{
        for (auto pBoard : *fDetectorContainer)
        {
                this->CheckFastCommands(pBoard, sFastCommand, filename);
        }
}
...
void PSROHHybridTester::ReadRefAddrBRAM (int iRefBRAMAddr)
{
        for (auto pBoard : *fDetectorContainer)
        {
                this->ReadRefAddrBRAM(pBoard, iRefBRAMAddr);
        }
}
...
void PSROHHybridTester::ReadCheckAddrBRAM (int iCheckBRAMAddr)
{
        for (auto pBoard : *fDetectorContainer)
        {
                this->ReadCheckAddrBRAM(pBoard, iCheckBRAMAddr);
        }
}
...
void PSROHHybridTester::PSROHInputsDebug()
{
    for (auto pBoard : *fDetectorContainer)
    {
     ...
     else {LOG(ERROR) <<"Wrong option!"<<std::endl; exit(1);}
     // stop
        LOG(INFO) << BOLDBLUE << "Do you want to stop test? [y/n]" << RESET;
     std::cin >> Answer;
        while (Answer != 'y') {
             LOG(INFO) << BOLDBLUE << "Do you want to stop test? " << RESET;
     }
     fBeBoardInterface->WriteBoardReg (pBoard, "fc7_daq_ctrl.physical_interface_block.debug_blk.stop_input", 1);
        std::this_thread::sleep_for (std::chrono::microseconds (100) );
        //results
     ...
     std::vector<std::string> RegisterTable = {
                {"fc7_daq_stat.physical_interface_block.debug_blk_counter00"},
    ...
    std::map< std::string, std::string > RegisterAlias = {
               {"fc7_daq_stat.physical_interface_block.debug_blk_counter00","l_fcmd_cic"},
