...
ThrMinimization::ThrMinimization(const char* fileRes, const char* fileReg, size_t rowStart, size_t rowStop, size_t colStart,
                                 size_t colStop, size_t nPixels2Inj, size_t nEvents, size_t nEvtsBurst, float targetOccupancy,
                                 size_t ThrStart, size_t ThrStop)
    : PixelAlive(fileRes, "", rowStart, rowStop, colStart, colStop, (rowStop - rowStart + 1) * (colStop - colStart + 1), nEvents,
                 nEvtsBurst, false)
    , fileRes(fileRes)
    , fileReg(fileReg)
    , rowStart(rowStart)
    ...
    for (const auto cBoard : *fDetectorContainer)
        for (const auto cModule : *cBoard)
            for (const auto cChip : *cModule)
                theThrContainer.at(cBoard->getIndex())
                    ->at(cModule->getIndex())
                    ->at(cChip->getIndex())
                    ->getSummary<RegisterValue, EmptyContainer>()
                    .fRegisterValue
                    = static_cast<RD53*>(cChip)->getReg("Vthreshold_LIN");
        ...
        for (const auto cModule : *cBoard)
            for (const auto cChip : *cModule)
                LOG(INFO) << BOLDGREEN << "\t--> Average threshold for [board/module/chip = " << BOLDYELLOW << cBoard->getId()
                          << "/" << cModule->getId() << "/" << cChip->getId() << BOLDGREEN << "] is " << BOLDYELLOW
...
void ThrMinimization::bitWiseScan(const std::string& dacName, uint32_t nEvents, const float& target, uint16_t startValue,
                                  uint16_t stopValue)
{
    uint16_t numberOfBits = (stopValue != 0 ? log2(stopValue - startValue) + 1
                                            : static_cast<BeBoard*>(fDetectorContainer->at(0))
                                                  ->fModuleVector.at(0)
        ...
        for (auto cModule : *cBoard)
            for (auto cChip : *cModule) {
                ...
                maxDACcontainer.at(cBoard->getIndex())
                    ...
                    ->getSummary<RegisterValue, EmptyContainer>()
                    .fRegisterValue
                    = (stopValue != 0 ? stopValue : RD53::setBits(numberOfBits)) + 1;

                bestContainer.at(cBoard->getIndex())
                    ->at(cModule->getIndex())
                    ->at(cChip->getIndex())
                    ->getSummary<OccupancyAndPh, EmptyContainer>()
                    .fPh
                    = 0;
            }
        ...
        for (const auto cBoard : *fDetectorContainer)
            for (auto cModule : *cBoard)
                for (auto cChip : *cModule) {
                    ...
                    this->fReadoutChipInterface->WriteChipReg(static_cast<RD53*>(cChip), dacName,
                                                              midDACcontainer.at(cBoard->getIndex())
                                                                  ->at(cModule->getIndex())
                                                                  ->at(cChip->getIndex())
                                                                  ->getSummary<RegisterValue, EmptyContainer>()
            ...
            for (auto cModule : *cBoard)
                for (auto cChip : *cModule) {
                    ...
                    float newValue = cChip->getSummary<GenericDataVector, OccupancyAndPh>().fOccupancy
                                     * ((rowStop - rowStart + 1) * (colStop - colStart + 1)) * nEvents;
