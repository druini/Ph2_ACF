...
SCurve::SCurve(const char* fileRes, size_t rowStart, size_t rowEnd, size_t colStart, size_t colEnd, size_t nPixels2Inj,
               size_t nEvents, size_t startValue, size_t stopValue, size_t nSteps)
    : fileRes(fileRes)
    , rowStart(rowStart)
    , rowEnd(rowEnd)
    , colStart(colStart)
    ...
    , stopValue(stopValue)
    , nSteps(nSteps)
    , histos(nEvents, nSteps, startValue, stopValue)
    , Tool()
{
    // ########################
    ...
    for (const auto cBoard : *fDetectorContainer)
        for (const auto cModule : *cBoard)
            for (const auto cChip : *cModule) {
                ...
                for (auto row = 0; row < RD53::nRows; row++)
                    for (auto col = 0; col < RD53::nCols; col++) {
                        for (auto i = 0; i < dacList.size() - 1; i++)
                            measurements[i + 1] = (detectorContainerVector[i + 1]
                                                       ->at(cBoard->getIndex())
                                                       ->at(cModule->getIndex())
                                                       ->at(cChip->getIndex())
                ...
                theThresholdAndNoiseContainer->normalizeAndAverageContainers(fDetectorContainer,
                                                                             fChannelGroupHandler->allChannelGroup(), 1);
                LOG(INFO) << BOLDGREEN << "\t--> Average threshold for [board/module/chip = " << BOLDYELLOW << cBoard->getId()
                          << "/" << cModule->getId() << "/" << cChip->getId() << BOLDGREEN << "] is " << BOLDYELLOW << std::fixed
                          << std::setprecision(1)
                          << theThresholdAndNoiseContainer->at(cBoard->getIndex())
                                 ->at(cModule->getIndex())
                                 ->at(cChip->getIndex())
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "SCurve_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
//                 theOccupancy.push_back(new TH2F(myString.str().c_str(), myString.str().c_str(), nSteps, startValue - VCalOffset,
//                                                 stopValue - VCalOffset, nEvents / 2 + 1, 0, 1 + 2. / nEvents));
//                 theOccupancy.back()->SetXTitle("#DeltaVCal");
//                 theOccupancy.back()->SetYTitle("Efficiency");

//                 myString.clear();
//                 myString.str("");
//                 myString << "CanvaScurve_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "Noise1D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "CanvasNo1D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "Threshold1D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "CanvasTh1D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "Noise2D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
//                 theNoise2D.push_back(new TH2F(myString.str().c_str(), myString.str().c_str(), RD53::nCols, 0, RD53::nCols,
//                                               RD53::nRows, 0, RD53::nRows));
//                 theNoise2D.back()->SetXTitle("Columns");
//                 theNoise2D.back()->SetYTitle("Rows");

//                 myString.clear();
//                 myString.str("");
//                 myString << "CanvasNo2D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "Threshold_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
//                 theThreshold2D.push_back(new TH2F(myString.str().c_str(), myString.str().c_str(), RD53::nCols, 0, RD53::nCols,
//                                                   RD53::nRows, 0, RD53::nRows));
...
//                 myString.clear();
//                 myString.str("");
//                 myString << "CanvasTh2D_Board" << std::setfill('0') << std::setw(2) << +cBoard->getIndex() << "_Mod"
//                          << std::setfill('0') << std::setw(2) << +cModule->getIndex() << "_Chip" << std::setfill('0')
//                          << std::setw(2) << +cChip->getIndex();
...
//                             for (auto i = 0; i < dacList.size(); i++)
//                                 theOccupancy[index]->Fill(dacList[i] - VCalOffset, detectorContainerVector[i]
//                                                                                        ->at(cBoard->getIndex())
//                                                                                        ->at(cModule->getIndex())
//                                                                                        ->at(cChip->getIndex())
//                                                                                        ->getChannel<Occupancy>(row, col)
...
//                         if (theThresholdAndNoiseContainer->at(cBoard->getIndex())
//                                 ->at(cModule->getIndex())
//                                 ->at(cChip->getIndex())
//                                 ->getChannel<ThresholdAndNoise>(row, col)
//                                 .fNoise
//                             != 0) {
//                             theThreshold1D[index]->Fill(theThresholdAndNoiseContainer->at(cBoard->getIndex())
//                                                             ->at(cModule->getIndex())
//                                                             ->at(cChip->getIndex())
//                                                             ->getChannel<ThresholdAndNoise>(row, col)
//                                                             .fThreshold);
//                             theNoise1D[index]->Fill(theThresholdAndNoiseContainer->at(cBoard->getIndex())
//                                                         ->at(cModule->getIndex())
//                                                         ->at(cChip->getIndex())
//                                                         ->getChannel<ThresholdAndNoise>(row, col)
//                                                         .fNoise);
//                             theThreshold2D[index]->SetBinContent(col + 1, row + 1,
//                                                                  theThresholdAndNoiseContainer->at(cBoard->getIndex())
//                                                                      ->at(cModule->getIndex())
//                                                                      ->at(cChip->getIndex())
//                                                                      ->getChannel<ThresholdAndNoise>(row, col)
//                                                                      .fThreshold);
//                             theNoise2D[index]->SetBinContent(col + 1, row + 1,
//                                                              theThresholdAndNoiseContainer->at(cBoard->getIndex())
//                                                                  ->at(cModule->getIndex())
//                                                                  ->at(cChip->getIndex())
//                                                                  ->getChannel<ThresholdAndNoise>(row, col)
...
//         theAxis.push_back(
//             new TGaxis(myPad->GetUxmin(), myPad->GetUymax(), myPad->GetUxmax(), myPad->GetUymax(),
...
//             RD53chargeConverter::VCAl2Charge(theThreshold1D[i]->GetBinLowEdge(theThreshold1D[i]->GetNbinsX())), 510, "-"));
...
//             RD53chargeConverter::VCAl2Charge(theNoise1D[i]->GetBinLowEdge(theNoise1D[i]->GetNbinsX()), true), 510, "-"));
...
void SCurve::FillHisto() { 
    for (auto i = 0; i < dacList.size(); i++)
        histos.fill_occupancy(*detectorContainerVector[i], dacList[i]);
    histos.fill_threshold_noise(*theThresholdAndNoiseContainer); 
}
