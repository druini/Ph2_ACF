#include "LpGBTTester.h"

using namespace Ph2_HwDescription;
using namespace Ph2_HwInterface;
using namespace Ph2_System;

LpGBTTester::LpGBTTester() : Tool() {}

LpGBTTester::~LpGBTTester() {}

void LpGBTTester::Initialise(){}
void LpGBTTester::Start(int currentRun)
{
  LOG(INFO) << BOLDBLUE << "Starting LpGBT Tester" << RESET;
  Initialise();
}
void LpGBTTester::Stop()
{
  LOG(INFO) << BOLDBLUE << "Stopping LpGBT Tester" << RESET;
  dumpConfigFiles();
  Destroy();

}
void LpGBTTester::Pause(){}
void LpGBTTester::Resume(){}

void LpGBTTester::InjectULInternalPattern(uint32_t pPattern)
{
...
void LpGBTTester::InjectULExternalPattern(uint8_t pPattern)
{
...
void LpGBTTester::InjectDLInternalPattern(uint8_t pPattern)
{
    for(auto cBoard: *fDetectorContainer)
    {
        for(auto cOpticalGroup: *cBoard)
...
bool LpGBTTester::TestI2CMaster(const std::vector<uint8_t>& pMasters)
{
...
void LpGBTTester::TestADC(const std::vector<std::string>& pADCs, uint32_t pMinDACValue, uint32_t pMaxDACValue, uint32_t pStep)
{
#ifdef __USE_ROOT__
    for(auto cBoard: *fDetectorContainer)
    {
...
void LpGBTTester::SetGPIOLevel(const std::vector<uint8_t>& pGPIOs, uint8_t pLevel)
{
  for(auto cBoard: *fDetectorContainer)
  {
    for(auto cOpticalGroup: *cBoard)
