...
void GainOptimization::bitWiseScan (const char* dacName, float targetCharge, float targetToT)
{
  ...
  for (auto i = 0; i < numberOfBits; i++)
	...
	for (auto cModule : *cBoard)
	  for (auto cChip : *cModule)
	    if ((targetToT - output->at(cBoard->getIndex())->at(cModule->getIndex())->at(cChip->getIndex())->getSummary<GainAndIntercept,GainAndIntercept>().fIntercept) /
		output->at(cBoard->getIndex())->at(cModule->getIndex())->at(cChip->getIndex())->getSummary<GainAndIntercept,GainAndIntercept>().fGain > targetCharge)
