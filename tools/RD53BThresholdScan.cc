...
typename RD53BThresholdScan<Flavor>::OccupancyMap RD53BThresholdScan<Flavor>::run(Task progress) const {
    ...
    Base::for_each_chip([&] (auto* chip) {
        ...
        occ[chip] = xt::zeros<double>({vcalBins.size(), size(0), size(1)});
    });

    param("injectionTool"_s).configureInjections();

    std::vector<tool_result_t<RD53BInjectionTool<Flavor>>> injResults(vcalBins.size());
    ...
    for (size_t frame = 0; frame < nFrames; ++frame) {
        ...
        // for (size_t i = 0, vcalHigh = param("vcalMed"_s) + param("vcalRange"_s)[0]; i < nSteps; ++i, vcalHigh += param("vcalStep"_s)) {
            ...
            param("injectionTool"_s).inject(injResults[i]);
            progress.update(double(frame * vcalBins.size() + i) / (nFrames * vcalBins.size()));
        }
    }

    for (size_t i = 0; i < vcalBins.size(); ++i) {
        for (const auto& item : param("injectionTool"_s).occupancy(injResults[i])) {
            xt::view(occ[item.first], i, xt::all(), xt::all()) = 
...
template <class Flavor>
std::array<ChipDataMap<xt::xtensor_optional<double, 2>>, 2> RD53BThresholdScan<Flavor>::analyze(const OccupancyMap& result) const {
    ...
    Base::for_each_chip([&] (auto* chip) {
        ...
        auto enabled = xt::view(param("injectionTool"_s).usedPixels() && chip->injectablePixels(), rowRange(), colRange());
...
template <class Flavor>
void RD53BThresholdScan<Flavor>::draw(const OccupancyMap& result) {
    Base::createRootFile();
    ...
    auto thresholdAndNoise = analyze(result);
    ...
    // for (const auto& item : occMap) {
        ...
        double maxNoise = 1.1 * xt::amax(xt::value(noise))();

        Base::drawHist(xt::filter(xt::value(noise), valid), "Noise Distribution", 100, 0, maxNoise, "Delta VCAL");
    });
}
