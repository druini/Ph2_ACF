...
#include "RD53ThEqual.h"

ThEqual::ThEqual(const char* fName, size_t rStart, size_t rEnd, size_t cStart, size_t cEnd, size_t nPix, size_t nEvts, size_t startValue, size_t stopValue, size_t nSteps, float targetTh) :
  fileName(fName),
  rowStart(rStart),
  rowEnd(rEnd),
  colStart(cStart),
...
void ThEqual::InitHisto()
{
  ...
  for (const auto& cBoard : fBoardVector)
      ...
      for (const auto& cChip : cFe->fChipVector)
	{
	  ...
	  myString.clear();
	  myString.str("");
          myString << "ThEqual_Board" << std::setfill ('0') << std::setw (2) << +cBoard->getBeId()
...
void ThEqual::Save()
{
  ...
  theCanvasTh1D->Print("ThEqualTh1D.svg");
  theCanvasNo1D->Print("ThEqualNo1D.svg");
  theCanvasTh2D->Print("ThEqualTh2D.svg");
  theCanvasNo2D->Print("ThEqualNo2D.svg");
}


void ThEqual::ComputeStats(std::vector<float>& measurements, size_t offset, float& nHits, float& mean, float& rms)
{
  float mean2  = 0;
  float weight = 0;
  mean         = 0;
