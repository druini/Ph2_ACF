...
void SSAPedeNoise::cleanContainerMap()
{
    for(auto container : fSCurveOccupancyMap) fRecycleBin.free(container.second);
    fSCurveOccupancyMap.clear();
}

void SSAPedeNoise::Initialise (bool pAllChan, bool pDisableStubLogic)
{
    fDisableStubLogic = pDisableStubLogic;
    fChannelGroupHandler = new SSAChannelGroupHandler();//This will be erased in tool.resetPointers()
...
void SSAPedeNoise::sweepSCurves ()
{
...
void SSAPedeNoise::measureNoise ()
{
    sweepSCurves ();
    extractSSAPedeNoise ();
    produceSSAPedeNoisePlots();
}

void SSAPedeNoise::Validate ( uint32_t pNoiseStripThreshold, uint32_t pMultiple )
{
        ...
        for(auto cOpticalGroup : *cBoard)
        {
            for ( auto cFe : *cOpticalGroup )
            {
                ...
                for ( auto cSSA : *cFe )
                {
                    ...
                    {
                        float occupancy = theOccupancyContainer.at(cBoard->getIndex())->at(cOpticalGroup->getIndex())->at(cFe->getIndex())->at(cSSA->getIndex())->getChannel<Occupancy>(iChan).fOccupancy;
                        if( occupancy > float ( pNoiseStripThreshold * 0.001 ) )
                        {
                            ...
                            LOG (INFO) << RED << "Found a noisy channel on SSA " << +cSSA->getId() << " Channel " << iChan  << " with an occupancy of " << occupancy << "; setting offset to " << +0xFF << RESET ;
                        }
                    }

                    fReadoutChipInterface->WriteChipMultReg (static_cast<SSA*>(cSSA), cRegVec);
                }
            }
        }
...
uint16_t SSAPedeNoise::findPedestal (bool forceAllChannels)
{
    ...
    for (auto cBoard : *fDetectorContainer)
    {
        for(auto cOpticalGroup : *cBoard)
        {
            for ( auto cFe : *cOpticalGroup )
            {
                for ( auto cSSA : *cFe )
                {
                    uint16_t tmpVthr = static_cast<ReadoutChip*>(cSSA)->getReg("Bias_THDAC");
                    cMean+=tmpVthr;
                    ++nSSA;
                }
            }
...
void SSAPedeNoise::measureSCurves (uint16_t pStartValue)
{
    ...
    while (! (cAllZero && cAllOne) )
    {
        ...
        this->setDacAndMeasureData("Bias_THDAC", cValue, fEventsPerPoint);
...
void SSAPedeNoise::extractSSAPedeNoise ()
{
...
void SSAPedeNoise::produceSSAPedeNoisePlots()
{
    #ifdef __USE_ROOT__
...
void SSAPedeNoise::setThresholdtoNSigma (BoardContainer* board, uint32_t pNSigma)
{
    for ( auto opticalGroup : *board)
    {
        for ( auto hybrid : *opticalGroup)
        {
            for ( auto chip : *hybrid )
            {
                ...
                if (pNSigma > 0) LOG (INFO) << "Changing Threshold on SSA " << +cSSAId << " by " << cDiff << " to " << cPedestal + cDiff << " Bias_THDAC units to supress noise!" ;
                else LOG (INFO) << "Changing Threshold on SSA " << +cSSAId << " back to the pedestal at " << +cPedestal ;
...
void SSAPedeNoise::writeObjects()
{
    #ifdef __USE_ROOT__
...
void SSAPedeNoise::ConfigureCalibration()
{
    CreateResultDirectory ( "Results/Run_SSAPedeNoise" );
}

void SSAPedeNoise::Start(int currentRun)
{
...
void SSAPedeNoise::Pause()
{
}

void SSAPedeNoise::Resume()
{
}
