...
RD53Interface::RD53Interface(const BeBoardFWMap& pBoardMap)
    : ReadoutChipInterface(pBoardMap)
{
}
...
bool RD53Interface::WriteChipReg(Chip* pChip, const std::string& pRegNode, const uint16_t data, bool pVerifLoop)
{
    ...
    if (strcmp(pRegNode.c_str(), "GLOBAL_PULSE") == 0)
        pRD53->encodeCMD(cRegItem.fAddress, cRegItem.fValue, pRD53->getChipId(), RD53CmdEncoder::GLOB_PULSE, false,
                         serialSymbols);
        ...
        if (pVerifLoop == true) {
            ...
            if (strcmp(pRegNode.c_str(), "PIX_PORTAL") == 0)
                pixMode = this->ReadRD53Reg(pRD53, "PIX_MODE")[0].second;
            if (pixMode == 0)
                outputDecoded = this->ReadRD53Reg(pRD53, pRegNode);
            if ((strcmp(pRegNode.c_str(), "PIX_PORTAL") == 0) && (pixMode == 0))
                row = this->ReadRD53Reg(pRD53, "REGION_ROW")[0].second;
            ...
            if ((pixMode == 0)
                ...
                if ((strcmp(pRegNode.c_str(), "VCAL_HIGH") == 0) || (strcmp(pRegNode.c_str(), "VCAL_MED") == 0))
                    usleep(DEEPSLEEP); // @TMP@
                return true;
            }
        }
    }
...
bool RD53Interface::WriteChipMultReg(Chip* pChip, const std::vector<std::pair<std::string, uint16_t> >& pVecReg, bool pVerifLoop)
{
    ...
    for (const auto& cReg : pVecReg) {
        ...
        if (strcmp(cReg.first.c_str(), "GLOBAL_PULSE") == 0)
            ...
            pRD53->encodeCMD(cRegItem.fAddress, cRegItem.fValue, pRD53->getChipId(), RD53CmdEncoder::RESET_BCR, false,
                             serialSymbols);
        else if (strcmp(cReg.first.c_str(), "RESET_EVTCTR") == 0)
            pRD53->encodeCMD(cRegItem.fAddress, cRegItem.fValue, pRD53->getChipId(), RD53CmdEncoder::RESET_ECR, false,
                             serialSymbols);
...
void RD53Interface::WriteRD53RegLong(RD53* pRD53, const std::string& pRegNode, const std::vector<uint32_t>& dataVec, size_t nCmd)
{
    ...
    for (auto i = 0u; i < nCmd; i++) {
        ...
        pRD53->encodeCMD(pRD53->getRegItem(pRegNode).fAddress, pRD53->getRegItem(pRegNode).fValue, pRD53->getChipId(),
                         RD53CmdEncoder::WRITE, true, serialSymbols, &subDataVec);
    }
...
void RD53Interface::WriteRD53Mask(RD53* pRD53, bool doSparse, bool doDefault, bool pVerifLoop)
{
    ...
    if (doDefault == true)
        mask = pRD53->getPixelsMaskDefault();
    else
        mask = pRD53->getPixelsMask();
    ...
    for (auto col = 128; col < 272; col += 2) {
        uint16_t row_;
        ...
        for (auto row = 0u; row < RD53::nRows; row++) {
             data =  /* RD53PixelEncoder::HIGHGAIN | */ static_cast<uint16_t>((*mask)[col].Enable[row])
                   | (static_cast<uint16_t>((*mask)[col].InjEn[row]) << RD53PixelEncoder::NBIT_PIXEN)
                   | (static_cast<uint16_t>((*mask)[col].HitBus[row])
                      << (RD53PixelEncoder::NBIT_PIXEN + RD53PixelEncoder::NBIT_INJEN))
                   | (static_cast<uint16_t>((*mask)[col].TDAC[row])
                      << (RD53PixelEncoder::NBIT_PIXEN + RD53PixelEncoder::NBIT_INJEN + RD53PixelEncoder::NBIT_HITBUS));

            data = data
                   | (( /* RD53PixelEncoder::HIGHGAIN | */ static_cast<uint16_t>((*mask)[col + 1].Enable[row])
                       | (static_cast<uint16_t>((*mask)[col + 1].InjEn[row]) << RD53PixelEncoder::NBIT_PIXEN)
                       | (static_cast<uint16_t>((*mask)[col + 1].HitBus[row])
                          << (RD53PixelEncoder::NBIT_PIXEN + RD53PixelEncoder::NBIT_INJEN))
                       | (static_cast<uint16_t>((*mask)[col + 1].TDAC[row])
                          << (RD53PixelEncoder::NBIT_PIXEN + RD53PixelEncoder::NBIT_INJEN + RD53PixelEncoder::NBIT_HITBUS)))
                      << (NBIT_CMD / 2));

            // data = /* RD53PixelEncoder::HIGHGAIN */ 0 | static_cast<uint16_t>((*mask)[col].Enable[row])
            //        | (static_cast<uint16_t>((*mask)[col].InjEn[row]) << RD53PixelEncoder::NBIT_PIXEN)
            //        | (static_cast<uint16_t>((*mask)[col].HitBus[row])
            //           << (RD53PixelEncoder::NBIT_PIXEN + RD53PixelEncoder::NBIT_INJEN))
            //        | (static_cast<uint16_t>((*mask)[col].TDAC[row])
            ...
            // data = data
            //        | (( /* RD53PixelEncoder::HIGHGAIN */ 0 | static_cast<uint16_t>((*mask)[col + 1].Enable[row])
            //            | (static_cast<uint16_t>((*mask)[col + 1].InjEn[row]) << RD53PixelEncoder::NBIT_PIXEN)
            //            | (static_cast<uint16_t>((*mask)[col + 1].HitBus[row])
            //               << (RD53PixelEncoder::NBIT_PIXEN + RD53PixelEncoder::NBIT_INJEN))
            //            | (static_cast<uint16_t>((*mask)[col + 1].TDAC[row])
            ...
            if (doSparse == true) {
                if (((*mask)[col].Enable[row] == 1) || ((*mask)[col + 1].Enable[row] == 1)) {
                    // LOG (INFO) << "Enabling pixel: " << row << ", " << col;
