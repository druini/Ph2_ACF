...
{
	BackendAlignmentInterface::BackendAlignmentInterface(const BeBoardFWMap& pBoardMap) :
	BeBoardInterface(pBoardMap) 
	{
		fObject.fHybrid=0;
        fObject.fChip=0; 
        fObject.fLine=0; 
	...
	bool BackendAlignmentInterface::GetStatus(BeBoard* pBoard)
    {
        ...
        {
            LOG(INFO) << "\t\t Done: " << +fStatus.fDone << ", PA FSM: " << BOLDGREEN << fPhaseFSMStateMap[fStatus.fPhaseAlignmentFSMstate] << RESET << ", WA FSM: " << BOLDGREEN << fWordFSMStateMap[fStatus.fWordAlignmentFSMstate] << RESET;
    ...
    void BackendAlignmentInterface::SendCommand( BeBoard* pBoard )
    {
    	this->WriteBoardReg(pBoard, "fc7_daq_ctrl.physical_interface_block.phase_tuning_ctrl", encodeCommand() );
	}
    ...
    void BackendAlignmentInterface::SelectLine( uint8_t pLine )
	{
		fObject.fLine = pLine ; 
    }
	//
