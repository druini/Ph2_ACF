...
bool RD53lpGBTInterface::WriteReg(Chip* pChip, uint16_t pAddress, uint16_t pValue, bool pVerifLoop)
{
    ...
    if(pVerifLoop == false) return true;

    unsigned int nAttempts = 0;
    uint8_t      cReadBack = RD53lpGBTInterface::ReadReg(pChip, pAddress);
    while((cReadBack != pValue) && (nAttempts < RD53lpGBTconstants::MAXATTEMPTS))
    {
        fBoardFW->WriteOptoLinkRegister(pAddress, pValue, pVerifLoop);
        cReadBack = RD53lpGBTInterface::ReadReg(pChip, pAddress);
        nAttempts++;
    }
