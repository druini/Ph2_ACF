...
float RD53FWInterface::calcVoltage(uint32_t senseVDD, uint32_t senseGND)
{
  ...
  bool RD53FWInterface::RunPRBStest (bool given_time, unsigned long long frames_or_time, uint16_t hybrid_id, uint16_t chip_id)
  {
    ...
    // Initialize number of frames and time to run
    unsigned long long frames2run = 0;
    unsigned           time2run   = 0;
    int                fps        = 3.5E7;

    // if given a time to run, convert into an approximate number of frames assuming the above FPS
    if(given_time){
      time2run   = frames_or_time;
      frames2run = (unsigned long long)time2run*fps;
      LOG (INFO) << GREEN << "Running " << time2run << "s will send about " << frames2run << " frames" << RESET;
    }

    // otherwise find the time it takes to run N frames with the given FPS
    else{
      frames2run = frames_or_time;
      time2run   = (unsigned)frames2run/fps;
      LOG (INFO) << GREEN << "Running " << frames2run << " frames will take about " << time2run << "s" << RESET;
    }
    
    // Configure number of printouts and calculate the frequency of printouts
    int n_prints = 10; // Only an indication, the real number of printouts will be driven by the length of the time steps
    unsigned time_per_step = std::min(std::max((unsigned)time2run/n_prints, (unsigned)1), (unsigned)3600); // length of a time step. The runtime of the PRBS test will have a precision of one step (at most 1h and at least 1s).
    ...
    WriteStackReg({
        {"user.ctrl_regs.prbs_frames_to_run_low",  lowFrames},
    ...
    LOG (INFO) << GREEN << "After conversion into 2x32 bits: low number: " << lowFrames << " -- high number: " << highFrames << RESET;

    // Start PRBS
    WriteStackReg({
        {"user.ctrl_regs.PRBS_checker.start_checker",1},
	{"user.ctrl_regs.PRBS_checker.start_checker",0}});
	...
	LOG (INFO) << GREEN << "Now running for " << (unsigned)time_per_step*(idx+1) << "s (" << std::setprecision(3) << percent_done << "% done)" << RESET;      
	LOG (INFO) << GREEN << "Current BER counter: " << ReadReg("user.stat_regs.prbs_ber_cntr") << RESET;
	if(given_time) run_done = ((unsigned)time_per_step*(idx+1) >= time2run);
	else           run_done = (current_frame >= frames2run);
	idx++;
      }
    ...
    LOG (INFO) << GREEN << "Final number of PRBS frames sent: " << frameCounter << RESET;
     
    // Read PRBS BER counter
    LOG (INFO) << GREEN << "Final BER counter: " << ReadReg("user.stat_regs.prbs_ber_cntr") << RESET;
