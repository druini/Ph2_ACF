...
uint32_t RD53FWInterface::getBoardInfo()
{
    ...
    LOG(INFO) << BOLDBLUE << "FW version : " << BOLDYELLOW << cVersionMajor << "." << cVersionMinor << BOLDBLUE
              << " -- Build version : " << BOLDYELLOW << cVersionBuild << BOLDBLUE
              << " -- Firmware date (yyyy/mm/dd) : " << BOLDYELLOW << cFWyear << "/" << cFWmonth << "/" << cFWday << RESET;
...
void RD53FWInterface::ConfigureBoard(const BeBoard* pBoard)
{
    ...
    for (const auto& cModule : pBoard->fModuleVector) {
        ...
        LOG(INFO) << BOLDBLUE << "Enabled " << BOLDYELLOW << pBoard->fModuleVector.size() << BOLDBLUE << " chips for module "
                  << BOLDYELLOW << cModule->getIndex() << RESET;
    }
...
void RD53FWInterface::WriteChipCommand(std::vector<uint32_t>& data, unsigned int nCmd)
{
    ...
    for (auto i = 0u; i < nCmd; i++) {
        ...
        if (nCmd != 1)
            stackRegisters.push_back({ "user.ctrl_regs.fast_cmd_reg_1.cmd_strobe", 0 }); // @TMP@
    }

    WriteStackReg(stackRegisters);
}

std::vector<std::pair<uint16_t, uint16_t> > RD53FWInterface::ReadChipRegisters(std::vector<uint32_t>& data, uint8_t chipID,
                                                                               uint8_t filter)
{
    // ##############################
    ...
    if (chipID < nActiveChns) {
        ...
        if (nodeBlocks < BLOCKS2READ) {
            LOG(ERROR) << BOLDRED << "Number of register blocks to read (" << BOLDYELLOW << BLOCKS2READ << BOLDRED
                       << ") exceds FIFO lenght " << BOLDYELLOW << nodeBlocks << RESET;
        ...
        for (auto i = 0u; i < regFIFO.size(); i++) {
            ...
            auto first = (regFIFO[i] >> RD53RegFrameEncoder::NBIT_VALUE)
                         & static_cast<uint32_t>(RD53::setBits(RD53RegFrameEncoder::NBIT_ADDRESS));
            auto status = (regFIFO[i] >> (RD53RegFrameEncoder::NBIT_VALUE + RD53RegFrameEncoder::NBIT_ADDRESS))
                          & static_cast<uint32_t>(RD53::setBits(RD53RegFrameEncoder::NBIT_STATUS));
            auto id = (regFIFO[i] >> (RD53RegFrameEncoder::NBIT_VALUE + RD53RegFrameEncoder::NBIT_ADDRESS
                                      + RD53RegFrameEncoder::NBIT_STATUS))
                      & static_cast<uint32_t>(RD53::setBits(RD53RegFrameEncoder::NBIT_CHIPID));

            if (status != 0)
                LOG(ERROR) << BOLDRED << "Status error in chip register readback: " << BOLDYELLOW << std::hex << +status
                           << std::dec << BOLDRED << " from chip ID: " << BOLDYELLOW << std::hex << +id << std::dec << RESET;

            outputDecoded.push_back(std::pair<uint16_t, uint16_t>(first, second));
        }
    }
    else
        LOG(ERROR) << BOLDRED << "Request to read chip registers from a non active channel: " << BOLDYELLOW << +chipID << BOLDRED
                   << " (active channels are " << BOLDYELLOW << "0" << BOLDRED << "--" << BOLDYELLOW << +nActiveChns - 1
                   << BOLDRED << ")" << RESET;
...
void RD53FWInterface::PrintFWstatus()
{
    ...
    LOG(INFO) << BOLDBLUE << "Fast CMD block trigger source: " << BOLDYELLOW << fastCMDReg << BOLDBLUE
              << " (1=IPBus, 2=Test-FSM, 3=TTC, 4=TLU, 5=External, 6=Hit-Or, "
    ...
    LOG(INFO) << BOLDBLUE << "Fast CMD block check if configuraiton registers have been set: " << BOLDYELLOW << fastCMDReg
              << RESET;
...
bool RD53FWInterface::InitChipCommunication()
{
    ...
    if (RD53::countBitsOne(bitReg) == auroraReg) {
        LOG(INFO) << BOLDGREEN << "\t--> Aurora channels up number as expected: " << BOLDYELLOW << RD53::countBitsOne(bitReg)
                  << RESET;
        return true;
    }
    LOG(ERROR) << BOLDRED << "\t--> Aurora channels up number less than expected: " << BOLDYELLOW << RD53::countBitsOne(bitReg)
               << RESET;
    return false;
}

void RD53FWInterface::Start() { SendBoardCommand("user.ctrl_regs.fast_cmd_reg_1.start_trigger"); }

void RD53FWInterface::Stop() { SendBoardCommand("user.ctrl_regs.fast_cmd_reg_1.stop_trigger"); }

void RD53FWInterface::Pause() { SendBoardCommand("user.ctrl_regs.fast_cmd_reg_1.stop_trigger"); }

void RD53FWInterface::Resume() { SendBoardCommand("user.ctrl_regs.fast_cmd_reg_1.start_trigger"); }
...
void RD53FWInterface::ReadNEvents(BeBoard* pBoard, uint32_t pNEvents, std::vector<uint32_t>& pData, bool pWait)
{
    ...
    do {
        ...
        // ####################
        this->Start();
        while (ReadReg("user.stat_regs.trigger_cntr").value() < pNEvents * (1 + this->localCfgFastCmd.trigger_duration))
            usleep(SHALLOWSLEEP);
        ...
        if (events.size() != this->localCfgFastCmd.n_triggers * (1 + this->localCfgFastCmd.trigger_duration)) {
            LOG(ERROR) << BOLDRED << "Sent " << this->localCfgFastCmd.n_triggers * (1 + this->localCfgFastCmd.trigger_duration)
                       << " triggers, but collected " << events.size() << " events" << BOLDYELLOW << " --> retry" << RESET;
            retry = true;
            continue;
        }
    ...
    if (retry == true) {
        LOG(ERROR) << BOLDRED << "Reached the maximum number of trals (" << BOLDYELLOW << MAXTRIALS << BOLDRED
                   << ") without success" << RESET;
        pData.clear();
    }
}
...
{
    WriteStackReg(
        { { "system.ctrl_2.fmc_pg_c2m", 0 }, { "system.ctrl_2.fmc_l8_pwr_en", 0 }, { "system.ctrl_2.fmc_l12_pwr_en", 0 } });
}

void RD53FWInterface::TurnOnFMC()
{
    WriteStackReg(
        { { "system.ctrl_2.fmc_l12_pwr_en", 1 }, { "system.ctrl_2.fmc_l8_pwr_en", 1 }, { "system.ctrl_2.fmc_pg_c2m", 1 } });
...
{
    WriteStackReg({ { "user.ctrl_regs.reset_reg.readout_block_rst", 1 }, { "user.ctrl_regs.reset_reg.readout_block_rst", 0 } });
}

void RD53FWInterface::ChipReset()
{
    WriteStackReg({ { "user.ctrl_regs.reset_reg.scc_rst", 1 },
...
{
    WriteStackReg({ { "user.ctrl_regs.fast_cmd_reg_1.ipb_bcr", 1 }, { "user.ctrl_regs.fast_cmd_reg_1.ipb_bcr", 0 } });
}
...
std::vector<RD53FWInterface::Event> RD53FWInterface::DecodeEvents(const std::vector<uint32_t>& data, uint8_t& evtStatus)
{
    ...
    for (auto i = 0u; i < event_start.size(); i++) {
        ...
        if (evt.evtStatus != RD53FWEvtEncoder::GOOD)
            evtStatus |= evt.evtStatus;
        else {
    ...
    for (auto i = 0u; i < events.size(); i++) {
        ...
        for (auto j = 0u; j < evt.chip_events.size(); j++) {
            ...
            for (const auto& region_data : evt.chip_events[j].data) {
                LOG(INFO) << "Column: " << region_data.col << ", Row: " << region_data.row << ", ToTs: [" << +region_data.tots[0]
                          << "," << +region_data.tots[1] << "," << +region_data.tots[2] << "," << +region_data.tots[3] << "]"
                          << RESET;
            }
        }
    }
...
RD53FWInterface::Event::Event(const uint32_t* data, size_t n)
{
    ...
    // #########################
    bool dummy_size;
    std::tie(tlu_trigger_id, data_format_ver, dummy_size)
        = unpack_bits<RD53FWEvtEncoder::NBIT_TRIGID, RD53FWEvtEncoder::NBIT_FMTVER, RD53FWEvtEncoder::NBIT_DUMMY>(data[1]);
    ...
    for (auto i = 4u; i < n; i += 2)
        if (data[i] >> (RD53FWEvtEncoder::NBIT_ERR + RD53FWEvtEncoder::NBIT_HYBRID + RD53FWEvtEncoder::NBIT_FRAMEHEAD
                        + RD53FWEvtEncoder::NBIT_L1ASIZE)
            == RD53FWEvtEncoder::FRAME_HEADER)
    ...
    for (auto i = 0u; i < chip_start.size(); i++) {
        ...
        if (chip_events[i].evtStatus != RD53EvtEncoder::CGOOD)
            evtStatus |= chip_events[i].evtStatus;
    }
}
...
RD53FWInterface::ChipFrame::ChipFrame(const uint32_t data0, const uint32_t data1)
{
    std::tie(error_code, hybrid_id, chip_id, l1a_data_size)
        = unpack_bits<RD53FWEvtEncoder::NBIT_ERR, RD53FWEvtEncoder::NBIT_HYBRID, RD53FWEvtEncoder::NBIT_FRAMEHEAD,
                      RD53FWEvtEncoder::NBIT_L1ASIZE>(data0);
...
void RD53FWInterface::ConfigureDIO5(const DIO5Config* cfg)
{
    ...
    WriteStackReg({ { "user.ctrl_regs.ext_tlu_reg1.dio5_en", (uint32_t)cfg->enable },
