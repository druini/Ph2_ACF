                ...
                for(const auto cChip: *cHybrid){
					for(int input=0;input<4096;input+=1){
						...
						for(int variable=0;variable<1;variable++){
							...
							if(input>pow(2.0,pRD53RegMap[writeVar[variable]].fBitSize)-1)
								continue;
							if(input==0){
							...
							if(input>1){
								if(((DACcode[variable][input]>0 && DACcode[variable][input-1]==0)||(DACcode[variable][input-1]>0 && DACcode[variable][input]==0)) && fitStart[variable]==0){
									fitStart[variable]=VMUXvolt[variable][input-1];
									//The last two with inverse structure don't have the correct starting point
								}
								if(((DACcode[variable][input]==4095 && DACcode[variable][input-1]<4095)||(DACcode[variable][input]<4095 && DACcode[variable][input-1]==4095)) && fitEnd[variable]==0)
									fitEnd[variable]=VMUXvolt[variable][input];
								if(fitEnd[variable]==0 && input == pow(2.0,pRD53RegMap[writeVar[variable]].fBitSize)-1 - 1)
									fitEnd[variable] = VMUXvolt[variable][input];
							}
							
							if(input>0){
								DNLcode[variable][input] = DACcode[variable][input]-DACcode[variable][input-1]-1;
							}else{
