                ...
                for(const auto cChip: *cHybrid){
					for(int input=0;input<100;input++){
						...
						for(int variable=0;variable<1;variable++){
							...
							if(input>1){
								if(((ADCcode[variable][input]>0 && ADCcode[variable][input-1]==0)||(ADCcode[variable][input-1]>0 && ADCcode[variable][input]==0)) && fitStart[variable]==0){
									fitStart[variable]=VMUXvolt[variable][input-1];
								}
								if(((ADCcode[variable][input]==4095 && ADCcode[variable][input-1]<4095)||(ADCcode[variable][input]<4095 && ADCcode[variable][input-1]==4095)) && fitEnd[variable]==0)
							...
							if(input>0){
								DNLcode[variable][input] = ADCcode[variable][input]-ADCcode[variable][input-1]-1;
							}else{
