...
void InitParameters(const SystemController& sc,
                    ...
                    size_t& nEvents, size_t& nEvtsBurst, size_t& NTRIGxL1A, std::string& INJtype,

                    size_t& ROWstart, size_t& ROWstop, size_t& COLstart, size_t& COLstop, size_t& nPixelInj,

                    size_t& LatencyStart, size_t& LatencyStop,

                    size_t& VCALstart, size_t& VCALstop, size_t& VCALnsteps,

                    size_t& targetCharge, size_t& KrumCurrStart, size_t& KrumCurrStop,

                    size_t& display, size_t& chipRegDefault)
{
    ...
    for (const auto& cBoard : sc.fBoardVector) {
        ...
        for (const auto& cModule : cBoard->fModuleVector)
            for (const auto& cChip : cModule->fReadoutChipVector) {
                ...
                cfgFastCmd.trigger_source
                    = (hitOr == true ? RD53FWInterface::TriggerSource::HitOr : RD53FWInterface::TriggerSource::FastCMDFSM);
                ...
                if (type == "Digital") {
                    ...
                    LOG(ERROR) << BOLDRED << "Option non recognized " << type << RESET;
                ...
                RD53Board->getLoaclCfgFastCmd()->fast_cmd_fsm.delay_after_first_cal
                    = cfgFastCmd.fast_cmd_fsm.delay_after_first_cal;
                RD53Board->getLoaclCfgFastCmd()->fast_cmd_fsm.delay_after_second_cal
                    = cfgFastCmd.fast_cmd_fsm.delay_after_second_cal;
...
void ConfigureExtClkTrig(SystemController& sc)
{
    ...
    for (const auto& cBoard : sc.fBoardVector) {
        ...
        // # Configuring DIO5 #
        // ####################
        LOG(INFO) << GREEN << "Configuring DIO5 for external trigger and external clock for board " << BOLDYELLOW
                  << cBoard->getBeBoardId() << RESET;
...
int main(int argc, char** argv)
{
    ...
    cmd.defineOption("calib",
                     "Which calibration to run [latency pixelalive noise scurve gain threqu gainopt thrmin]. Default: pixelalive",
    ...
    // # Configure software #
    // ######################
    size_t nEvents, nEvtsBurst, NTRIGxL1A, ROWstart, ROWstop, COLstart, COLstop, nPixelInj, LatencyStart, LatencyStop, VCALstart,
        VCALstop, VCALnsteps, targetCharge, KrumCurrStart, KrumCurrStop, display, chipRegDefault;
    std::string INJtype;
    InitParameters(cSystemController, nEvents, nEvtsBurst, NTRIGxL1A, INJtype, ROWstart, ROWstop, COLstart, COLstop, nPixelInj,
                   LatencyStart, LatencyStop, VCALstart, VCALstop, VCALnsteps, targetCharge, KrumCurrStart, KrumCurrStop, display,
                   chipRegDefault);
    ...
    // ######################################
    if (nPixelInj == 0)
        nPixelInj = (ROWstop - ROWstart + 1) * (COLstop - COLstart + 1)
                    / (ROWstop - ROWstart + 1 + ((ROWstop - ROWstart + 1) > NROW_CORE ? NROW_CORE : 0));
    ...
    // ######################
    if (extClkTrg == true)
        ConfigureExtClkTrig(cSystemController);
        
    int VCALOffset = static_cast<RD53*>(cSystemController.fBoardVector[0]->fModuleVector[0]->fReadoutChipVector[0])->getReg("VCAL_MED");
    ...
    if (whichCalib == "latency") {
        ...
        PixelAlive pa(ROWstart, ROWstop, COLstart, COLstop, nPixelInj, nEvents, nEvtsBurst, true);
        ...
        PixelAlive pa(ROWstart, ROWstop, COLstart, COLstop, (ROWstop - ROWstart + 1) * (COLstop - COLstart + 1), nEvents,
                      nEvtsBurst, false);
        pa.Inherit(&cSystemController);
        pa.Run();
        pa.Analyze();
        ...
        SCurve sc(fileName.c_str(), ROWstart, ROWstop, COLstart, COLstop, nPixelInj, nEvents, VCALstart - VCALOffset, VCALstop - VCALOffset, VCALnsteps);
        ...
        if (chipRegDefault == true)
            chipConfig = "./CMSIT_RD53.txt";
        else
            chipConfig = "./CMSIT_RD53_" + runNumber + ".txt";
        ThrEqualization te(fileName.c_str(), chipConfig.c_str(), ROWstart, ROWstop, COLstart, COLstop, nPixelInj,
                           nEvents * VCALnsteps, nEvents, output);
        ...
        if (chipRegDefault == true)
            ...
            chipConfig = "./CMSIT_RD53_" + runNumber + ".txt";
        GainOptimization go(fileName.c_str(), chipConfig.c_str(), ROWstart, ROWstop, COLstart, COLstop, nPixelInj, nEvents,
                            VCALstart, VCALstop, VCALnsteps, RD53chargeConverter::Charge2VCal(targetCharge), KrumCurrStart,
                            KrumCurrStop);
    ...
    if (fileRunNumberOut.is_open() == true)
        fileRunNumberOut << runNumber << std::endl;
